...
public class SwiftDeviceCalendarPlugin: NSObject, FlutterPlugin {
    ...
    private func createCalendar(_ call: FlutterMethodCall, _ result: FlutterResult) {
        ...
        let defaultCalendarSource = eventStore.defaultCalendarForNewEvents?.source
        let iCloud = eventStore.sources.first(where: { $0.sourceType == .calDAV && $0.title.lowercased() == "icloud"})
        let local = eventStore.sources.first(where: { $0.sourceType == .local })
        var source = local
        if (source != nil && source!.title.lowercased() != defaultCalendarSource!.title.lowercased() && iCloud != nil) {
            source = iCloud
        }
        
        if (source == nil && iCloud != nil) {
            source = iCloud
        }
